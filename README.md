...
「我的 → 导入 JSON 恢复」读取 `schema: held-ledger-export, version: 1` 的备份，确认后替换当前台账。空台账也可以导出、导入和重启保存。上传照片默认压缩为 360×360 JPEG，随 JSON 内嵌保存；导出会将可读取的图片引用转成内嵌图片，保留已有内嵌图片，省略自动生成的占位图。图片无法读取时会列出资产并中止导出，需要先重新添加照片，避免生成悄然缺图的备份。备份保存的是应用中的图片，不是上传前的全尺寸原图。
